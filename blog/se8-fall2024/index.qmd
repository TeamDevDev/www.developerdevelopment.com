...
An example they use in the book is that Google used to only use 2 space indents for their Python code but the outside community used 4. Eventually they realized the Google standard did not fit compared to the outside standard. The more your code interacts with the outside world the more staying consistent with outside standards matter.

##### Avoid Error-prone and Surprising Constructs

Even if current developers understand a confusing construct there is no guarrentee the future will. These non-obvious features can cause misuse and introduce bugs. 
...
Good style guides have rrules that do three things:
- Rules to avoid dangers
- Rules to enforce best practices 
- Riles to ensure consistency

Dangerous rules are things that allow bugs to creep in such as using lamba expressions or hard to use langauge features.
