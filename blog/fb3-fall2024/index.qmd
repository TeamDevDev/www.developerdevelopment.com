...
**Activity: What do you think the benefits as well as tradeoffs are with fuzzing programs?**
...
Fuzzers have helped people for a long time find errors that they need to fix. Some programs come with a max length for inputs, and when it is triggered it is called buffer overflows. 
* In languages without exceptions, errors are often indicated by special return codes. For example in C, getchar() reads characters from input and returns EOF when no more input is available. 
...
* If you give a random number input that might be too large, is less than what you want or is negative fuzzing can run into issues.
...
**Activity: How do you think the runner relates to fuzzing? What do you think this runner does?**
...
Overall Fuzzing is a great way to check programs. By discovering bugs related to how a program handles inputs, fuzzers can be very useful when testing code. Fuzzers can detect many types of errors including buffer overflows, missing error checks, and the precense of rogue numbers.
