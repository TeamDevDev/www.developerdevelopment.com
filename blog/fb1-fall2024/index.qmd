...
description: <em>What is software testing and why is it importtant?</em>
toc: true
page-layout: full
---
...
[GatorGrade](https://github.com/GatorEducator/gatorgrade) with the technical
skills from the [Introduction to Software
...
method to check our code as well as develop a way to "grade" others
automatically. Software testing is key to our work so it is important to
understand the basics of testing and why we do it.
...
Imagine a function `my_sqrt()` that calculates the square root of a number. If we have a program `sqrt_program()` that takes user input as a string:
...
But what if the input is `-1`? It could cause problems because `my_sqrt()` might not handle negative numbers properly. To avoid issues like infinite loops, we can use a timeout:
...
- **Value Validation**: If the conversion succeeds, the function then checks if the number is negative. If it is, it prints "Illegal Number" to indicate that the input is not suitable for square root calculation.
- **Computation**: If the input is a valid non-negative number, the function proceeds to compute the square root using `my_sqrt()` and prints the result.
...
Think about how we can verify the correctness of the inputs to our features. Remember to validate the input so we do not run into unexpected errors down the line!
