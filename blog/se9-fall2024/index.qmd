...
Some changes to the code base are automated. While these changes are written by a machine and not a human they still must be reviewed in the same manner as all other code reviews. Though with LSC(large-scale changes) no one person can veto a change, otherwise they would take too long to implement.
