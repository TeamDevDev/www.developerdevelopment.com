...
function can handle inputs that could potentially trick it into thinking they
are true.
...
when it comes to testing `Chasten` with a wide verity of inputs.

## Use Cases

We could potentially utilize the practices of mutation-based fuzzing in the
testing of our tool, `Chasten`. `Chasten` requires an input to be formatted in
a specific way; therefore, the use of randomly generated inputs would only be
useful for testing our parser. To ensure that we could adequately use fuzzing
to test all aspects of our program, we would __have__ to use mutation-based
fuzzing.

This could be done by implementing the same methods the chapter uses. First, we
would need to find a valid input that makes it passed the parser. Then, we
would need to run that input through a function such as `mutate` or
`multi_mutate` to add mutations to the input. Then we would be able run the
mutated version of the input through the program to test for any errors.
