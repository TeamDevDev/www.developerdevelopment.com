...
def api(
    ...
    if not status.startswith("2") or payload.get("success") is False:
        messages = "; ".join(
            error_item.get("message", "?")
            for error_item in payload.get("errors", [])
        ) or "Cloudflare returned success=false"
...
def hostnames_for(config: dict) -> list[str]:
    """Return the configured hostnames with the apex first."""
    configured = config.get(
        "hostnames", [config["apex"], f"www.{config['apex']}"]
    )
    return [config["apex"]] + [
        hostname for hostname in configured if hostname != config["apex"]
    ]


def netlify_apex_ips_for(config: dict) -> set[str]:
    """Return configured Netlify apex addresses for this site."""
    addresses = config.get("netlify_apex_ips", [NETLIFY_APEX_IP])
...
def load_config(path: str) -> dict:
    """Read and validate the per-site TOML configuration file."""
    ...
    hostnames = hostnames_for(config)
    if not isinstance(hostnames, list) or not hostnames or not all(
        isinstance(hostname, str) and hostname for hostname in hostnames
    ):
        die(f"config key 'hostnames' must be a non-empty list: {path}")
    if config["apex"] not in hostnames:
        die(f"config hostnames must include the apex '{config['apex']}'")
    netlify_apex_ips = netlify_apex_ips_for(config)
    if not netlify_apex_ips or not all(
        isinstance(address, str) and address for address in netlify_apex_ips
    ):
        die(f"config key 'netlify_apex_ips' must contain IP addresses: {path}")
    return config
...
def is_netlify_record(
    record: dict, netlify_apex_ips: set[str] | None = None
) -> bool:
    """Is this DNS record part of the old Netlify setup?"""
    apex_ips = netlify_apex_ips or {NETLIFY_APEX_IP}
    if record.get("type") == "A" and record.get("content") in apex_ips:
        return True
    content = str(record.get("content", "")).lower()
    return record.get("type") == "CNAME" and content.endswith("netlify.app")
...
def stage_check(config: dict) -> None:
    ...
    if token:
        ...
        for name in hostnames_for(config):
            records = dns_records(token, zone, name)
            for record in records:
                flag = (
                    " <-- Netlify"
                    if is_netlify_record(record, netlify_apex_ips_for(config))
                    else ""
                )
                info(
...
def worker_domains(
    token: str, account_id: str, zone: str, hostname: str
) -> list:
    """Return Worker custom domains matching one hostname."""
    query = urlencode({"hostname": hostname, "zone_id": zone})
    response = api(
        "GET", f"/accounts/{account_id}/workers/domains?{query}", token
    )
    return response.get("result", [])
...
def attach_domain(
    ...
    hostname: str,
    worker: str,
    netlify_apex_ips: set[str],
) -> None:
    """Replace Netlify DNS and attach one hostname as a custom domain.

    The Workers Custom Domains API does not support an override flag for
    existing DNS records. Cloudflare requires conflicting records to be
    removed before the custom domain is attached, so do that explicitly.
    The stage_domain function has already saved a rollback snapshot before
    this function is called.
    """
    records = dns_records(token, zone, hostname)
    for record in records:
        if is_netlify_record(record, netlify_apex_ips):
            api(
                "DELETE",
    ...
    # These are the fields supported by PUT /accounts/{id}/workers/domains.
    body = {
        "hostname": hostname,
        "service": worker,
        "zone_id": zone,
    ...
    domains = worker_domains(token, account_id, zone, hostname)
    if not any(
        domain.get("hostname") == hostname
        and domain.get("service") == worker
        for domain in domains
    ):
        raise CloudflareApiError(
            f"Cloudflare did not report {hostname} attached to {worker}", []
        )

    remaining = dns_records(token, zone, hostname)
    if any(
        is_netlify_record(record, netlify_apex_ips) for record in remaining
    ):
        raise CloudflareApiError(
            f"Netlify DNS records still exist for {hostname} after binding",
            [],
        )
    info(f"bound {hostname} to {worker}; custom domain verified")


def stage_domain(config: dict, assume_yes: bool = False) -> None:
    """Snapshot DNS, then bind the configured hostnames to the Worker."""
    print("[domain] binding custom domains (apex first)")
    hostnames = hostnames_for(config)
    hostname_summary = ", ".join(hostnames)
    confirm(
        f"This will replace the DNS records for {hostname_summary} so they "
        f"point at the Cloudflare Worker {config['worker_name']} instead of "
        "Netlify. A DNS snapshot is saved first so 'rollback' can restore "
        "them.",
        assume_yes,
    )
    token = get_token()
...
def stage_verify(config: dict) -> None:
    ...
    apex = config["apex"]
    failures = 0
    for hostname in hostnames_for(config):
    ...
    for name in hostnames_for(config):
        ...
        if (
            "netlify" in output.lower()
            or any(ip in output for ip in netlify_apex_ips_for(config))
        ):
            failures += 1
...
def stage_rollback(config: dict, assume_yes: bool = False) -> None:
    ...
    for name, records in state["records"].items():
        ...
        for record in records:
            # Preserve record-specific fields such as MX priority, CAA data,
            # and record settings while excluding Cloudflare metadata.
            body = {
                key: record[key]
                for key in (
                    "type",
