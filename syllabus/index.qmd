...
## Computer Science 203 Fall 2026
...
- **Discussion, Writing, and Software Engineering Sessions**
  - *Developing Professional Skills*: Monday, 1:30 PM -- 2:20 PM, Alden 109
    ...
    - **Assigned team of learners**: Prepare for and give the executive summary
  of the assigned chapters and demonstrate an affiliated software command-line tool
  - *Developing Technical Skills*: Wednesday, 1:30 PM -- 2:20 PM, Alden 109
    - **FB Book**: [Fuzzing Book](https://www.fuzzingbook.org/)
    - **DB Book**: [Debugging Book](https://www.debuggingbook.org/)
    - **Majority of the class**: Discuss executive summaries for chapters read last week
    - **Assigned team of learners**: Prepare for and give the executive summary
  of the assigned chapters and demonstrate an affiliated software command-line tool
  - *Developing as Developers*: Friday, 1:30 PM -- 2:20 PM, Alden 109
    - Publish a new executive summary for next week's technical skills
    - Publish a new executive summary for next week's professional skills
    - Confirm our team's readiness for next week's in-class discussions
    - **When necessary**: build and demonstrate software engineering project
    - **Alternating assessments completed during the class session**:
      - *Every other week i*: complete an in-class programming skill-check
      - *Every other week i+1*: complete an in-class knowledge-check
        - Complete a brief in-person assessment with the course instructor
        - Complete an online form-based assessment on your laptop
- **Collaborative Software Engineering**: Wednesday, 2:30 PM -- 4:00 PM, Alden 109
  - *Collaboratively implement and deploy into production a software tool*:
    - Sprint planning
    - Project implementation
    - Project demonstration
  - At the end of each module, assessment in the form of:
    - Executable examinations and/or
...
preparation for the in-class discussions that will be lead by a self-assigned
team of learners.
...
- **Week Two**: Anshu, Dylan, V, Ritesh (SE1) *and* Kaitlyn, Katie, Aidana, Eyra (FB1)
- **Week Three**: Logan, Eyra, Aidana (SE2) *and* Xander, Anshu (FB2)
- **Week Four**: Jake, Aidana, Anshu (SE3) *and* Lukas, Johny (FB3)
- **Week Five**: Mark, Benito, Rivan (SE4) *and* Logan, Abishek (FB4)
- **Week Six**: In-person assessments and executable examination one

#### Module Two Discussion Leads

- **Week Seven**: Fall break, no presentations by learners
- **Week Eight**: Katie, Ritesh, Kaitlyn (SE5) *and* Xander, Dylan (FB5)
- **Week Nine**: Abishek, Eyra (SE6) *and* V, Logan (FB6)
- **Week Ten**: Johny, Jake (SE7) *and* Mark, Benito, Rivan (FB7)
- **Week Eleven**: In-person assessments and executable examination two

#### Module Three Discussion Leads

- **Week Twelve**: Abishek, Johny, Lukas (SE8) *and* V, Kaitlyn (FB8)
- **Week Thirteen**: Mark, Benito, Rivan (SE9) *and* Katie, Ritesh (FB9)
- **Week Fourteen**: Thanksgiving break, no presentations by learners
- **Week Fifteen**: Xander, Lukas (SE10) *and* Dylan, Jake (DB1)
- **Week Sixteen**: Final examination week
...
- **Week One**: No reading assignment due to completion of onboarding tasks
...
- **Week Seven**: Fall break, no presentations by learners
- **Week Eight**: SE5 (Engineering for Equity and Documentation) and FB5 (Mutation-Based Fuzzing)
- **Week Nine**: SE6 (How to Lead a Team) and FB6 (Fuzzing with Grammars)
- **Week Ten**: SE7 (Leading at Scale) and FB7 (Efficient Grammar Fuzzing)
- **Week Eleven**: No reading assignment due to assessments

#### Module Three Reading Assignments

- **Week Twelve**: SE8 (Style Guides and Rules) and FB8 (Parsing Inputs)
- **Week Thirteen**: SE9 (Code Review) and FB9 (Reducing Failure-Inducing Inputs)
- **Week Fourteen**: Thanksgiving break, no presentations by learners
- **Week Fifteen**: SE10 (Testing Overview and Unit Testing) and DB1 (Debugging and Observing Executions)
...
parentheses. As a reminder, these are the abbreviations for the three
course textbooks that are freely available online:

- **SE Book**: [Software Engineering at Google](https://abseil.io/resources/swe-book)
- **FB Book**: [Fuzzing Book](https://www.fuzzingbook.org/)
- **DB Book**: [Debugging Book](https://www.debuggingbook.org/)
- *Additional Text book*: [Software Engineering: Standing on the Shoulders of Giants](https://www.swebook.org/)
...
Students who develop both **professionally** and **technically** are undertaking
the task of **developer development**, which is both the theme and goal of this
web site's content!
...
schedule](../schedule/index.qmd) multiple reading assignments in **SE**, **FB**
, **DB**. These course textbooks are connected to the course activities in these
ways:
...
- Install an appropriate version of both **FB** and **DB**
- Complete one-time setup and installation steps for the **FB** or **DB**
textbooks:
  - Create a virtual environment for the book's dependencies
  - Enter into (i.e., activate) the virtual environment for a textbook
  - Install all platform-dependent binaries for the book's dependencies (e.g.,
  `graphviz`)
  - Install the book's dependencies with `uv pip install -r requirements.txt`
  - Troubleshoot all the issues that arise during the installation of the book's
  dependencies (e.g., one or more missing dependencies cause a Jupyter Notebook
  cell to produce a stack trace when you press {{< kbd Shift-Enter >}} to run
  the cell)
- Perform these steps both when reading an article and writing an executive
summary:
  - Change into the directory that contains the contents of either **FB** or
  **DB**
  - Enter into the virtual environment for a textbook
  - Run a Jupyter Lab server that can host the textbook's Jupyter notebooks
  - Use the Jupyter Lab server to locate and execute notebooks for chapters in
  **FB** and **DB**
  - Watch the embedded video(s) and run all the providing source code segments
  - Discussion leads for a weekly reading assignment additionally complete these
  tasks:
    - Create a fork of the course web site into a publicly available GitHub
  repository
    - Coordinate with your discussion lead team members to write and revise an
  executive summary in the Quarto-based Markdown format
    - Use [Quarto](https://quarto.org/) to write and revise an executive
  summary for the reading assignment
    - Create a pull request from your fork of the course web site and coordinate
  its review with the members of your team and the course instructor
  - Regardless of whether they are present an **SE**, **FB**, or **DB** article,
  the team presenting members should collaboratively create a command-line tool,
  using Python and `uv`, that practically illustrates a key concepts in the
  assigned reading
  - The leadership team (i.e., the student technical leader(s) and/or the course
  ...
  Actions](https://docs.github.com/en/actions) and Cloudflare Workers to publish
  the web site in advance of the session when your team leads class
...
- [Software Engineering: Standing on the Shoulders of Giants](https://www.swebook.org/), by Tom Hastings
...
- **Software Engineering Knowledge-Checks**: 10%
...
participate in all the class and laboratory sessions, as outlined on the
[course schedule](../schedule/index.qmd). After either an unexcused absence or
a late attendance to either a class or a laboratory session, a student's weekly
class participation grade will be reduced. Students who need to miss class or
attend class late for an excused reason should communicate their situation to
the course instructor in a timely fashion. In accordance with [Department of
Computer and Information Science
Policies](https://www.cis.allegheny.edu/teaching/policies/), each time a student
misses one week of class (i.e., three class sessions and one laboratory
...
laboratory sessions, undertaking non-course-related activities like reading or
responding to email, viewing social media, or reading or creating other content
not connected to software engineering.
...
of GitHub, VS Code, and their laptop's software engineering tools (e.g., package
managers and testing programs). Students may use external sources, including
...
evaluations of both high-level and low-level knowledge in the field of software
engineering. All students should be prepared to answer questions that assess
their comprehension of both concepts and implementation details in the field of
software engineering. Unless the instructor provides access to a computational
device, the in-person assessments invite students to demonstrate their knowledge
of computer security without the aid of any external sources, including, but not
limited to, software tools running on a student laptop.
...
- **Software Engineering Knowledge-Checks**: The software engineering
knowledge-checks are in-class, online and in-person cumulative assessments
covering all prior technical material from the course sessions, as outlined on
the [course schedule](../schedule/index.qmd). Unless prior arrangements are
made with the instructor, all students should use their computer to take these
knowledge-checks on the scheduled date and to complete it in the stated
location while taking no more than the required amount of time. Designed to
prepare learners for the discussion of concepts during an in-person assessment,
each knowledge-check involves the completion of an automatically scored online,
...
their knowledge of software engineering without the aid of any external
...
Designed to prepare learners for an examination, each skill-check is an in-class
executable assessment that students complete through the use of GitHub, VS Code,
and the Python programming tools on their laptops. Students may use external
sources, including artificial intelligence coding assistants, during the
completion of a skill-check provided that they cite these sources and
explain, in detail, how they used them to complete the examination.

- **Professional Development Summaries**: Building on material in the
[Software Engineering at Google](https://abseil.io/resources/swe-book)
textbook (i.e., **SE**) and the content covered during that week's in-person
classroom sessions, the professional development tasks, namely the publication
of a team-based executive summary, both have the following goals: (i) help a
learner to demonstrate that they can remember the learned material by recalling
facts, basic concepts, and answers to questions presented in **SE** and (ii)
allow a learner to demonstrate an understanding of facts and ideas by
translating, interpreting, and contextualizing the main professional concepts
found in **SE**. In addition to publishing an executive summary on the course
web site, this category invites learners to prepare and release a command-line
software tool that connects to the chosen reading assignment in **SE**. A
student who successfully completes this assignment will have evidence of
completed work as individual commits, with appropriately labeled commit
messages, in the GitHub repository for both the professional development summary
and the command-line software tool.

- **Technical Development Summaries**: Building on material in the [Fuzzing
Book](https://www.fuzzingbook.org/) (i.e., **FB**) and [Debugging
Book](https://www.debuggingbook.org/) textbooks (i.e., **DB**) and the content
covered during that week's in-person classroom sessions, the technical
development tasks, namely the publication of a team-based executive summary,
have the following goals: (i) help a learner to show that they can remember the
learned material by recalling facts, basic concepts, and answers to questions
presented in **FB**/**DB** and (ii) allow a learner to demonstrate an
understanding of facts and ideas by interpreting and implementing the main
technical ideas presented in either **FB** or **DB**. In addition to publishing
an executive summary on the course web site, this category invites learners to
prepare and release a command-line software tool that connects to the chosen
reading assignment in **FB**/**DB**. A student who successfully completes this
assignment will have evidence of completed work as individual commits, with
appropriately labeled commit messages, in the GitHub repository for both the
professional development summary and the command-line software tool.
...
software tools. These projects also encourage students to use and build software
engineering tools to complete tasks like debugging and testing. The technical
work that a student completes during project development will take place in a
public GitHub repository and will be further documented and reflected on in a
private GitHub repository that the instructor will use to furnish assessments
and feedback to the student. Finally, the professional work that a student
undertakes as part of project development will occur in both an in-person
setting and through a public GitHub repository, with the use of a private GitHub
repository offering the opportunity for the student to document and reflect on
their contributions and for the instructor to offer detailed feedback and
assessments. A pull request created by a team of software engineers will not be
merged by the course instructor unless it has a passing build and all team
members can demonstrate its feature(s) and answer in-person questions about its
design, implementation, and use. Finally, a student who successfully completes
this project will have evidence of completed work as individual commits, with
appropriately labeled commit messages, in the GitHub repository for the software
tool.
...
- In collaboration with an assigned discussion lead partner(s), read an assigned
article in **SE**, **FB**, or **DB** (recall that the course features one
article from **SE** and one article from either **FB** or **DB** every week) and
write an executive summary, that should contain, at minimum, these components:
  ...
  executive summary should connect to prior executive summaries, to the ongoing
  work on the long-term software engineering projects, and the team-created
  command-line software tool.
  ...
  completion of the long-term software engineering projects. With connections to
  the all course tasks (e.g., project development), each action item should have
  a clear description of the steps that the author and/or the team should **stop
  ...
  Specifically, the source code segments must use Quarto and appear in fenced
  code blocks and be directly executable in GitHub Actions and capable of
  correctly producing output when the web site is built and deployed to
  Cloudflare Workers.
  - All executive summaries should have a commit message history that clearly
  explains the changes that every member of the team made to the technical and
  professional content content. Importantly, each of the discussion leaders for
  each executive summary should ensure that all commit messages adhere to the
  [conventional commit](https://www.conventionalcommits.org/en/v1.0.0/) format
  as evidenced by prior commits to this site's GitHub repository.
- All executive summaries must be complemented by a GitHub repository that
contains a command-line software tool that is relevant to the assigned reading.
The GitHub repository for the assigned reading should be available through the
organization for the current edition of the software engineering course.
- Each team of learners assigned to complete an executive summary should
schedule time to meet with the course instructor to discuss their assigned
article. For each the executive summary, the assigned partnership of students
must complete their work in advance of the class session during which the
content will be discussed, as outlined in the [course
schedule](../schedule/index.qmd). When an executive summary is not completed
according to the stated schedule or not completed at the agreed-on level of
quality, this will result in a reduction in the corresponding technical or
professional development score for the assigned students.
...
Classroom (i.e., cs50), and/or the [Developer Development Discord
...
and published on the Developer Development web site and the completed version of
each software project should be available in the project's public GitHub
repository. No credit will be awarded for any course work that a student submits
to the incorrect GitHub repository or web site. Unless special arrangements are
made with the instructor due to extenuating circumstances or a student uses a
token, no work will be accepted after the deadline.
...
score. You may not use a token to request the reassessment of revised work that
you completed as part of specific tasks in the professional development,
technical development, or project development categories. For instance, you may
not spend a token by asking the instructor to reevaluate one of the executive
summaries (i.e., the blog posts) that you publish on the Quarto-based web site.
Students also may not request to use a token to re-assess their team
participation, their work for a completed phase of a long-term software
engineering project, or the final examination. However, they may apply tokens
to the midterm examinations, the programming skill-checks, and the knowledge
checks. Outside of these three tokens, the instructor will not consider any
additional requests for deadline extensions or project reassessment. The
instructor will submit to a student the assessment for work revised as part of
token used at the end of a course module. Unless there are extenuating
circumstances, you may not spend any tokens in the last two weeks of the
semester. Students will gain access to a GitHub repository that both enables
them to receive assessments from the instructor while also sharing more details
about the process for requesting the use of a token.
...
will always be available to serve as guide for individual students, teams of
students, and the entire class, it is expected that students will volunteer to
lead and actively contribute to all class sessions. Only those students who have
prepared for class by reading the assigned material, writing the required
executive summaries, and reviewing the content written by other students will be
able to effectively participate in these class discussions. In order to help
students remain organized and to effectively prepare for classes, the course
instructor will maintain a [course schedule](../schedule/index.qmd) with
reading assignments, project descriptions, and discussion lead assignments for
...
instructor. Students should, within the bounds of the Honor Code, ask and answer
questions on the [Developer Development Discord
Server](https://discord.gg/aNfn8DNEMx). Instead of sending either an email or a
direct message, please request assistance from the instructor through public
Discord channels. Students who need the course instructor's assistance are
invited to schedule a meeting through the [course instructor's appointment
scheduler](https://www.gregorykapfhammer.com/schedule/) and come to the meeting
with all the details needed to discuss their question(s) about the developer
development process (e.g., technical development, professional development, and
project development).
...
Computer and Information Science announcements. Any content that a student wants
the instructor to assess (e.g., an executive summary or a contribution to a
long-term software engineering project) must be in a GitHub repository. Unless a
student needs to share private information with the instructor, they should not
use Discord to send him direct messages.
...
and code generators (e.g., artificial intelligence systems that leverage large
language models like GitHub Copilot, OpenCode, or Pi) and documentation sites
...
automatically generated by a software tool that leverages artificial
intelligence.
...
the class are responsible for all the source code and documentation submitted to
the GitHub repository that hosts the course projects, including any
tool-generated software artifacts. This means that all students should be able
to demonstrate and answer questions, during an in-person assessment, about any
technical or professional content, including that which was automatically
