...
The output shows that `cat` has succeeded for each random input. Although not
comprehensive, this does a good job at catching inputs that may not have been
expected. Note that, although this example is quite simple, restricting our
input to random sequences of bytes, `RandomFuzzer` could be implemented for any
kind of data structure and thus to any kind of program!

## Reflection

We have seen how fuzzing provides an automated way to provide creative inputs
to a program, given that we know the kind of data a program is expecting.
