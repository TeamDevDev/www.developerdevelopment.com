---
title: Schedule
layout: full
toc: true
---

## Schedule Overview

- **Week One**: Technical and Professional Onboarding
- **Module One**: *Introduction to Software Engineering Knowledge and Skills*
  - **Weeks Two through Five**: Establish your technical and professional skills
  - **Week Six**:
    - **Monday**: in-person assessment of individual developer development
    - **Wednesday Class Session**: in-person assessment of individual developer development
    - **Wednesday Laboratory Session**: take an executable examination
    - **Friday**: individual demonstration of project development
- **Module Two**: *Practical Software Engineering Knowledge and Skills*
  - **Weeks Seven through Ten**: Enhance your technical and professional skills
  - **Week Eleven**:
    - **Monday**: in-person assessment of individual developer development
    - **Wednesday**: in-person assessment of individual developer development
    - **Thursday**: take an executable examination during the laboratory session
    - **Friday**: individual demonstration of project development
- **Module Three**: *Advanced Software Engineering Knowledge and Skills*
  - **Weeks Eleven through Fifteen**: Enhance your technical and professional skills
  - **Week Sixteen**:
    - **Wednesday, December 8, 2026 at 7:00 PM**:
      - **Details**: Final Examination (Exam Code F)
      - **Demonstrate technical and professional skills in software engineering**:
        - *Technical Skill Assessment*:
          - Use GitHub and GitHub Actions
          - Use text editor and terminal window
          - Implement and test Python functions
          - Debug defective Python functions
          - Confirm output of Python functions
          - Covers all content in SE, FB, and DB
        - *Professional Skill Assessment*:
          - Answer questions during in-person discussion
          - Answer questions through a provided online form
          - Covers all content in SE, FB, and DB

::: {.callout-note appearance="minimal" title="Textbook Abbreviations" collapse="false"}

- SE: [Software Engineering at Google](https://abseil.io/resources/swe-book)
- FB: [Fuzzing Book](https://www.fuzzingbook.org/)
- DB: [Debugging Book](https://www.debuggingbook.org/)

:::

## Weekly Cadence

- {{< iconify fa6-solid people-line size=18px >}} **Monday**: Professional Development
  - *Discuss the team's executive summary and use their command-line software tool*:
    - Team-based discussion of the week's reading in SE
    - Individual and team-based preparation for next week's SE reading
- {{< iconify fa-solid laptop-code size=18px >}} **Wednesday**: Technical Development
  - *Discuss the team's executive summary and use their command-line software tool*:
    - Team-based discussion of prior week's reading in either FB or DB
    - Individual and team-based preparation for next week's FB or DB reading
- {{< iconify fa6-solid diagram-project size=18px >}} **Wednesday**: Project Development during laboratory session
  - *Collaboratively implement and deploy into production a software tool*:
    - Sprint planning
    - Project implementation
    - Project demonstration
  - At the end of each module, assessment in the form of:
    - Executable examinations and/or
    - In-person assessments
- {{< iconify game-icons team-idea size=18px >}} **Friday**: Developer Development
  - *Publish executive summaries to the Developer Development blog*:
    - Publication of next SE executive summary
    - Publication of next FB or DB executive summary
    - Demonstration of project development for the week
    - Confirmation of readiness for next week's in-class discussions
    - Assessment in the form of either:
    - Programming skill-checks
    - Software engineering knowledge-checks

::: {.callout-note appearance="minimal" title="Assessment Reminder" collapse="false"}

The Fall 2026 edition of Computer Science 203 has **Exam Code F**, which means
that the final examination will take place on **Tuesday, December 8, 2026 at
7:00 PM**. Completion of the final examination must adhere to all of the
policies in the Allegheny College Academic Bulletin and the Allegheny College
Faculty Handbook. The final executable examination is cumulative in regards to
the technical, professional, and project content previously covered between
week one and week sixteen. This list describes the content that will be covered
on the final executable examination, in-person assessment, and knowledge check.

- {{< iconify fa6-solid flag-checkered size=18px >}} **Tuesday, December 8, 2026 at 7:00 PM**: Executable Examination and In-Person Assessment Completed During the Designated Examination Period
  - *Demonstrate technical and professional skills in software engineering*:
    - Use GitHub and GitHub Actions
    - Use text editor and terminal window
    - Implement and test Python functions
    - Debug defective Python functions
    - Confirm output of Python functions
    - Covers all content in SE, FB, and DB
    - Answer questions during in-person assessment
    - Answer questions through a provided online form

:::
