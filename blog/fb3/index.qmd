...
fuzzing into the Chasten program could have many benefits. In Chasten we can
check for bugs with inputting code or using configure files. The use of fuzzing
could also simulate a user's behavior which would give the team an idea of bugs
that a user may encounter. We have to apply what we have learned about fuzzing
