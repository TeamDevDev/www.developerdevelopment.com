...
Of course, even simple grammar structures such as start symbols and expansion
rules can be effective for the
[chasten](https://github.com/AstuteSource/chasten) tool. By defining our start
symbols as several mathematical terms we can create a highly effective "grammar
fuzzing" tool for producing complex inputs, which we can utilize to test even
more inputs for the tool.

It is also worth noting that the test suite for
[chasten](https://github.com/AstuteSource/chasten) already uses `from
hypothesis_jsonschema import from_schema`. This leverages the
[hypothesis](https://github.com/HypothesisWorks/hypothesis) tool to
automatically generate inputs that adhere to a provided JSON schema.
